...
// System prompt for relevancy evaluation
...
// Answer relevancy check implementation
export const checkAnswerRelevancy = async (opts: {
  ...
  const { object: statementsResult } = await generateObject({
    ...
    schema: z.object({
      statements: z
        .array(z.string())
        .describe("Array of meaningful statements"),
    }),
  });
      ...
      verdicts: z
        .array(
          z.object({
            verdict: z
              .enum(["yes", "no", "unsure"])
              .describe("The verdict for this statement"),
  ...
  const totalScore = verdicts.reduce((sum, verdict) => {
    return sum + scores[verdict.verdict];
  }, 0);

  const averageScore = verdicts.length > 0 ? totalScore / verdicts.length : 0;
  ...
  return {
    ...
    metadata: {
      statements,
      verdicts,
      rationale: `Evaluated ${statements.length} statements. ${verdicts.filter((v) => v.verdict === "yes").length} relevant, ${verdicts.filter((v) => v.verdict === "unsure").length} partially relevant, ${verdicts.filter((v) => v.verdict === "no").length} not relevant.`,
    },
  };
};
